...
def create_new_license(product: str, expiration: str, description: str, license_type: str):
    # Move constant out
    fmt = "%Y-%m-%d %H:%M:%S %z"
    ...
    try:
        ...
        if license_type != LicenseType.perpetual.name and not expiration:
            raise Exception(f"Expiration date required for {license_type} license")

        expiration = datetime.strptime(expiration, fmt) if license_type != LicenseType.perpetual.name else None
...
def _generate_license():
    ...
    20 random character string (including dashes) in the following format ####-####-####-####.
